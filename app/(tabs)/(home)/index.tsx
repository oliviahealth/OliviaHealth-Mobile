...
export default function Index() {
  ...
  const filterSection = useCallback(
    <T extends { title: string; spotlight?: boolean }>(items?: T[]): T[] => {
      if (!items) return [];
  ...
  const video_spotlights_featured = useMemo(
    () => filterSection<IVideoSpotlights>(resources?.video_spotlights),
    [resources, filterSection],
  );

  const local_resources_featured = useMemo(
    () => filterSection<ILocalResources>(resources?.local_resources),
    [resources, filterSection],
  );

  const quick_tips_featured = useMemo(
    () => filterSection<IQuickTips>(resources?.quick_tips),
    [resources, filterSection],
  );

  const infographics_features = useMemo(
    () => filterSection<IInfographics>(resources?.infographics),
    [resources, filterSection],
  );
  ...
  const goToVideoSpotlight = (videoSpotlight: IVideoSpotlights) => {
    router.push({
      ...
      params: { videoSpotlight: JSON.stringify(videoSpotlight) },
    });
  };
  ...
  const goToLocalResource = (localResource: ILocalResources) => {
    router.push({
      ...
      params: { localResource: JSON.stringify(localResource) },
    });
  };
  ...
  const goToQuickTip = (quickTip: IQuickTips) => {
    router.push({
      ...
      params: { quickTip: JSON.stringify(quickTip) },
    });
  };
  ...
  const goToInfographic = (infographic: IInfographics) => {
    router.push({
      ...
      params: { infographic: JSON.stringify(infographic) },
    });
  };
          ...
          <View style={{ flex: 1, gap: 18 }}>
            ...
            {video_spotlights_featured.length > 0 && (
              <View style={{ marginTop: 16 }}>
                ...
                >
                  <Text
                    style={{ fontSize: 22, fontWeight: "500", color: "#000" }}
                  >
                    Video Spotlights
                  </Text>
                ...
                {video_spotlights_featured.map((elm) => (
                  ...
                  >
                    ...
                    <Pressable
                      onPress={() => goToVideoSpotlight(elm)}
                      hitSlop={8}
                    >
                      <Ionicons
